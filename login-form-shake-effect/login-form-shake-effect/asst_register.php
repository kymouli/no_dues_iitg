...
<!--        <th style="padding-right: 0px; padding-left: 0px; text-align: center;"></th>-->
 ...
 <?php
 if (!isset($_SESSION["name"]) || ($_SESSION["type"] != "asst_register"))
 {
...
//    $main_q1 = "SELECT * FROM `care_taker` WHERE name = '$main_name'";
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST")
   {
     ...
     foreach ($res as $row)
     {
       if (!$row[$main_type] && $row['care_taker'] && $row['warden'] && $row['gymkhana'])
       {
...
//    $i = 0;
   ...
   foreach ($res as $row)
   {
    if($row[$main_type] == 0 && $row['care_taker'] && $row['warden'] && $row['gymkhana'])
...
<!-- <div id="warden_table_approved" style="visibility:hidden;">
<table id="keywords" cellspacing="0" cellpadding="0" style="empty-cells: hide; margin-top: 22%; ">
    <thead style="font-size: 17px;">
      <tr>
        <th><span>Name</span></th>
        <th><span>Roll No.</span></th>
