...
<!--        <th style="padding-right: 0px; padding-left: 0px; text-align: center;"></th>-->
 ...
 <?php
 if (!isset($_SESSION["name"]) || ($_SESSION["type"] != "faculty"))
 {
...
//   $q1 = "SELECT * FROM `student` WHERE department = `".$_SESSION['department']."`";
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST")
   {
     ...
     foreach ($res as $row)
     {
       if (!$row[$main_name] && $row['department'] == $_SESSION['department'])
       {
         if (isset($_POST[$row["roll_num"]]))
         {
           sql_check($row["roll_num"],$conn,$main_name);
         }
       }
     }
...
//    $i = 0;
   ...
   foreach ($res as $row)
   {
    if(!$row[$_SESSION['name']] && $row['department'] == $_SESSION['department'])
