...
<!--        <th style="padding-right: 0px; padding-left: 0px; text-align: center;"></th>-->
 ...
 <?php
 if (!isset($_SESSION["name"]) || ($_SESSION["type"] != "warden"))
 {
...
//    $main_q1 = "SELECT * FROM `care_taker` WHERE name = '$main_name'";
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST")
   {
     ...
     foreach ($res as $row)
     {
       if (!$row[$main_type] && $row['hostel'] == $_SESSION['hostel'] && $row['care_taker'])
       {
...
//    $i = 0;
   ...
   foreach ($res as $row)
   {
    if($row[$main_type] == 0 && $row['hostel'] == $_SESSION['hostel'] && $row['care_taker'])
